...
public:
    ...
    */
    void odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        if (systemInitialized == false)
        {
            ...
            optParameters.relinearizeThreshold = 0.1;
            optParameters.relinearizeSkip = 1;
            optimizer = gtsam::ISAM2(optParameters);  // 优化器

            gtsam::NonlinearFactorGraph newGraphFactors; // 生成一个对象
            graphFactors = newGraphFactors;  // 图因子

            gtsam::Values NewGraphValues;  // 图的值
            graphValues = NewGraphValues;  // 图的值
          //  resetOptimization(); 把代码拿出来
        ...
        while (!imuQueOpt.empty())
        {
            ...
            if (imuTime < currentCorrectionTime - delta_t) //delta_t这个变量好像就一直没有动
            {
                // 两帧imu数据时间间隔
                double dt = (lastImuT_opt < 0) ? (1.0 / 500.0) : (imuTime - lastImuT_opt);
                // imu预积分数据输入：加速度、角速度、dt imuIntegratorOpt_这个东西是每次都能存进去吗
    ...
    */
    void imuHandler(const sensor_msgs::Imu::ConstPtr& imu_raw)
    {
        ...
        // 要求上一次imu因子图优化执行成功，确保更新了上一帧（激光里程计帧）的状态、偏置，预积分重新计算了 确保odometryHandler执行了一次
